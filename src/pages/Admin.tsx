      ...
      <div className="max-w-sm mx-auto space-y-6 px-4">
        ...
        <Accordion type="multiple" className="space-y-3">
          {[
                ...
                <>
                  ...
                  <div className="container space-y-3" style={{ height: 200 }}>
                    ...
                    <div className="bg-[#181C23] rounded-lg h-20 flex items-center justify-between px-4">
                      <span className="font-display tracking-wider">BOS</span>
                      <span className="font-body text-xs text-muted-foreground">vs</span>
                      <span className="font-display tracking-wider">MIA</span>
                    </div>
                    <div className="bg-[#181C23] rounded-lg h-20 flex items-center justify-between px-4">
                      <span className="font-display tracking-wider">MIL</span>
                      <span className="font-body text-xs text-muted-foreground">vs</span>
                      <span className="font-display tracking-wider">IND</span>
                    </div>
                  </div>
                </>
              ),
            ...
            {
              value: "header-mypicks",
              label: "DEMO HEADER — MY PICKS",
              content: (
                <>
                  <HeroBannerMinimal title="My Picks" showDemoAvatar />
                  <div className="container space-y-3" style={{ height: 200 }}>
                    <p className="font-body text-xs uppercase tracking-widest text-muted-foreground">8 picks · 12 pts</p>
                    <div className="bg-[#181C23] rounded-lg h-14 flex items-center justify-between px-4">
                      <span className="font-body text-sm">BOS in 5</span>
                      <span className="font-body text-xs text-primary">+3</span>
                    </div>
                    <div className="bg-[#181C23] rounded-lg h-14 flex items-center justify-between px-4">
                      <span className="font-body text-sm">DEN in 6</span>
                      <span className="font-body text-xs text-muted-foreground">pending</span>
                    </div>
                    <div className="bg-[#181C23] rounded-lg h-14 flex items-center justify-between px-4">
                      <span className="font-body text-sm">LAL in 7</span>
                      <span className="font-body text-xs text-destructive">0</span>
                    </div>
                  </div>
                </>
              ),
            },
            {
              ...
              label: "DEMO HEADER — LEADERBOARD",
              content: (
                <>
                  <HeroBannerMinimal title="Leaderboard" showDemoAvatar />
                  <div className="container space-y-2" style={{ height: 200 }}>
                    {[
                      { rank: 1, name: "Erik", pts: 24 },
                      { rank: 2, name: "Alexander", pts: 21 },
                      { rank: 3, name: "David", pts: 19 },
                      { rank: 4, name: "Fabian", pts: 15 },
                      { rank: 5, name: "Hannes", pts: 12 },
                    ].map((r) => (
                      <div key={r.rank} className="bg-[#181C23] rounded-lg h-10 flex items-center justify-between px-4">
                        <span className="font-body text-sm text-muted-foreground w-6">{r.rank}</span>
                        <span className="font-body text-sm flex-1">{r.name}</span>
                        <span className="font-display tracking-wider text-sm">{r.pts}</span>
                      </div>
                    ))}
                  </div>
                </>
