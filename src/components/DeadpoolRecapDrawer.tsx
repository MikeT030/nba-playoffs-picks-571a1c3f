...
export function DeadpoolRecapPanel({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    if (!active || finishedGames.length > 0 || gamesLoading) return;
    setGamesLoading(true);
    fetchFinishedGames()
      .then(setFinishedGames)
      .catch(() => {})
      .finally(() => setGamesLoading(false));
  }, [active, finishedGames.length, gamesLoading]);
  ...
  useEffect(() => {
    if (active && !autoGenerated && !summary && !loading) {
                ...
                {finishedGames.map((g) => {
                  ...
                  return (
                    <div key={id} className="flex items-center gap-2">
                      ...
                      <Label htmlFor={`src-${id}`} className="font-body text-sm">
      ...
      <DrawerContent className="bg-[#181C23] border-[#2B2F37]">
        <div className="mx-auto w-full max-w-md">
          ...
          <div className="px-4 pb-6">
            <DeadpoolRecapPanel active={open} />
          </div>
        </div>
      </DrawerContent>
    </Drawer>
