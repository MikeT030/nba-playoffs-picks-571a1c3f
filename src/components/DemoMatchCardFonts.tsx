...
interface DemoMatchCardFontProps {
  fontFamily: string;
  fontLabel: string;
}

/**
 * Variant of DemoMatchCard used to preview different sport fonts
 * applied to the team name abbreviations only. Annotated with the
 * font label at the top.
 */
const DemoMatchCardFont = ({ fontFamily, fontLabel }: DemoMatchCardFontProps) => {
  const away = {
    abbreviation: "SAC",
    name: "Kings",
    seed: 5,
  ...
  return (
    <div className="space-y-1">
      <p className="font-body text-xs text-primary uppercase tracking-wider px-1">
        {fontLabel}
      </p>
      <div className="block rounded-lg bg-[#1A1E24]/80 backdrop-blur-md select-none">
        ...
        <div className="p-5 flex items-center gap-4 pt-[20px] px-[10px]">
          ...
          <div className="flex-1 flex items-center gap-2 -translate-y-2">
            ...
            <div>
              <p className="tracking-wide text-lg" style={{ fontFamily }}>
                {away.abbreviation}
              </p>
...
const DemoMatchCardFonts = () => {
  ...
  return (
    <div className="space-y-4">
      {fonts.map((f) => (
        <DemoMatchCardFont key={f.fontLabel} {...f} />
      ))}
    </div>
  );
};

export default DemoMatchCardFonts;
