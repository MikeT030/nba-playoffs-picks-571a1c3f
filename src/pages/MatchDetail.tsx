        ...
        <div className="relative container py-6">
          ...
          <div className="flex items-center justify-between gap-5 pt-[4px]">
            ...
            <div className="text-center">
              {displayStatus === "live" && (
                <span className="inline-flex items-center text-[10px] font-body font-semibold uppercase tracking-widest -mt-10 -translate-y-0.5 whitespace-nowrap text-primary-foreground">
                  {match.time ?? ""}
                </span>
              )}
              {displayStatus === "final" && (
          ...
          {allPicks && allPicks.length > 0 ? (
            allPicks.map((pick) => {
              ...
              return (
                <div
                  key={pick.user_id}
                  className="flex items-center gap-4 rounded-lg border border-white/10 bg-[#22272E]/80 backdrop-blur-md p-4 transition-all duration-200 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5"
                >
