...
const MatchCard = ({ match }: MatchCardProps) => {
  ...
  const bet = isDemo ? { seriesId: match.id, winner: "MIA", gamesInSeries: 6 } : realBet;
  const points = isDemo ? 3 : realPoints;
    ...
    >
      ...
      <div className="p-5 flex items-center gap-4 pt-[20px]">
        ...
        <div className="text-center px-4">
          <div className="h-0">
            {match.status === "live" && (
              <span className="inline-flex items-center text-[10px] font-body font-semibold uppercase tracking-widest -translate-y-5 whitespace-nowrap text-[#fe953e]">
                {match.time ?? ""}
              </span>
            )}
            {match.status === "final" && (
